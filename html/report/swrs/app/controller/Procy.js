Ext.define('proauthRzAuthlog.controller.Procy', {
  ...
  //         select: this.showTab,
          ...
          'authloglist button[action=hidShw]':{
           ...
           'authloglist button[action=output]':{
           	click: this.output
          }
        });
    },
...
//调整视图高度
    ...
    var new_params={sdate:sdate,edate:edate,stime:stime,etime:etime,statway:statway,did:did};
  //      var new_params={sdate:sdate,edate:edate,stime:stime,etime:etime,statway:statway};
   ...
   if(sid==''){
    ...
    dwglkeycx: function(thisBtn){
    	...
    	var statWay=Ext.getCmp('statway').value;
    	if(statWay=='1'){
    		 tgrid.down('#corpColumn').hide();
    	   tgrid.down('#dataColumn').show();
    	}
    	else{

    	   
    	    tgrid.down('#corpColumn').show();
    	    tgrid.down('#dataColumn').hide();
      }
