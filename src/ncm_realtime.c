...
//    utMsgPrintMsg(psMsgHead);    
   	iReturn = utMsgGetSomeNVar(psMsgHead,8,"cond",      UT_TYPE_STRING, sizeof(keyword) - 1  ,keyword
   	                                      ,"del",       UT_TYPE_STRING, sizeof(keyword) - 1,  caDel
...
//    printf("[Num] UserName        DispName        Password       Ssid             ApName                       Mac               TermType      Dev     Os  Lang  Login Bytes          Start              Last \n");
   char caTg[32];
    while(psOnline) 
    {
    	sprintf(caTg,"%lu",psOnline->lGroupid);
    	if((strlen(caShopids)!=0)&&(!strstr(caShopids,caTg)))
    		goto toNext;
      if((strlen(psOnline->caName))==0||strcmp(psOnline->caName,"None")==0){
      	goto toNext;
      }
    		
    		memset(caApAddress,0,sizeof(caApAddress));
    		if(psOnline->lGroupid>0){
    		   strcpy(caApAddress, (char *)ncmGetDispNameByGid(psShmHead,psOnline->lGroupid));
    	  }
 //   	printf("pgroupid=%lu\n",psOnline->lGroupid);
								        ...
								        {	
								        	...
								        	{	
								        		if(strlen(keyword) > 0 )
								        		{
											        ...
											        utPltPutLoopVar(psDbHead,"on_apAddr",     iCount ,psOnline->psAp?psAp->caAddr:caApAddress);
												        if(psOnline->login == 1)
												        {
...
toNext:        psOnline = (ncPortalOnline *)pasHashNextS(&sHashInfo);
