...
function showPagingGrid(){
	...
	Ext.define('GridData', {
		...
		//定义表格字段
		fields: [
				{type: 'string', name: 'usermac'},
				{type: 'string', name: 'userip'},
				{type: 'string', name: 'onlinecou'},
				{type: 'string', name: 'succescon'},
				{type: 'string', name: 'late'},
				{type: 'string', name: 'lost'},
				{type: 'string', name: 'indenfy'},
	...
	store = Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url: '/pronline/Msg?FunName@ncwlan_portal_log',
			reader: {
				type:'json',
				root: 'topics',
    ...
    viewConfig: {forceFit:true},//自动延展每列的长度，使内容填满整个表格
		...
		tbar: Ext.create('Ext.PagingToolbar_dir', {
			...
			,'-',{
				text:'导出',
				iconCls:'exp',
				handler://downloadViewData(grid)
				function(){
           var win3 = Ext.widget('window', {
               title: '正在打开 用户与Portal交互日志.xls',
               closeAction: 'hide',
               x:400,
               y:32,
               width: 280,
