...
{  
	 ...
	 if(!utStrIsSpaces(caPlateid)) // 根据关键字查询
	 {
...
//	 if(!utStrIsSpaces(plateid)) // 根据关键字查询
//	 {
...
{  
   ...
   if(strlen(sort)==0){
	 ...
	 if(!utStrIsSpaces(plateid)) // 根据模版查询
	 {
//			snprintf(sql+strlen(sql), sizeof(sql)-strlen(sql), " and plateid = %lu ", atol(plateid));
	 }
	 iReturn = pasDbOneRecord(sql, 0, UT_TYPE_ULONG, 4, &lCount1);
  ...
  //  unsigned long lSgroupid;
    ...
    //printf("caName = %s\n", caName);
	  ...
	  if(strcmp(caKey, "update")==0){
	  	  ...
	  	  pasDbOneRecord(caTemp2, 0, UT_TYPE_STRING, sizeof(caImgpath1)-1, caImgpath1);
	  	  //printf("caImgpath1 = %s\n", caImgpath1);
	  	  if(strcmp(caImgpath, caImgpath1)==0){ // 是否选择上传图片, 否：原图片不变
	  	  	  //printf("*****************原图片不变******************\n");
	  	  	  if(strlen(caDispname)==0){
...
{  
   ...
   if(strlen(sort)==0){
	 ...
	 if(!utStrIsSpaces(areaid)) // 根据区域查询
	 {
...
//	  printf("lGrouid=%lu\n",lGroupid);
    ...
    strcpy(caName, pasDbReplaceQuote(caName)); // 转换字符串中带有的 ' 或 " 
	  ...
	  if(lShopId>0){
			  if(strcmp(caKey, "update")==0){
			  	  ...
			  	  pasDbOneRecord(caTemp2, 0, UT_TYPE_STRING, sizeof(caImgpath1)-1, caImgpath1);
			  	  printf("caImgpath1 = %s\n", caImgpath1);
			  	  if(strcmp(caImgpath1, caImgpath)==0){ // 是否选择上传图片, 否：原图片不变
			  	  	  printf("****************原图片不变************\n");
	 ...
	 iret = pasDbFetchInto(psCur,UT_TYPE_ULONG,  sizeof(long),          &lGroupid,
	  ...
	  while(0 == iret || 1405 == iret){
			 ...
			 switch(lFlags){
			 		...
			 		case 9:strcpy(s_flags, "删除");break;
			 		default:break;
			 }
	 ...
	 if(!utStrIsSpaces(caUpdate)) {
	 		if (strcmp(caUpdate, "update")==0){
	 	  	  ...
	 	  	  if(lCount>0){
	 	  	    ...
	 	  	    pasDbExecSqlF(caTemp);
//	            utWebDispMsg(iFd,psMsgHead,"ncmshop/ncm_shop_back_new.htm","AP已存在!","AP已存在!");
//	 	  	  		return 0;	
	 	  	  }else{
	 ...
	 if (!utStrIsSpaces(groupid)) // 每个商家自己查自己的
	 {
...
int Ncm_WebShopSWW_SetFunName(utShmHead *psShmHead)
{
		...
		pasSetTcpFunName("ncm_webcomindex2_list", ncm_webcomindex2_list, 0); // 广告管理-0508
		pasSetTcpFunName("ncm_webarea_list", ncm_webarea_list, 0);  // 区域信息-0509
		
		pasSetTcpFunName("ncm_webcomshop_dr_images", ncm_webcomshop_dr_images, 0);  // 商店管理图片上传-0521
		pasSetTcpFunName("ncm_webcomindex2_dr_images", ncm_webcomindex2_dr_images, 0);  // 广告管理图片上传-0521
		pasSetTcpFunName("ncm_webcomodity_dr_images", ncm_webcomodity_dr_images, 0);  // 商品管理图片上传-0521
		
		pasSetTcpFunName("ncm_webcomindex3_list", ncm_webcomindex3_list, 0); // 广告第二版-0609
		pasSetTcpFunName("ncm_webcomindex3_dr_images", ncm_webcomindex3_dr_images, 0);
		
		pasSetTcpFunName("ncm_websrvgroup_info", ncm_websrvgroup_info, 0); //单位管理信息-0613
		
		pasSetTcpFunName("ncm_webcomindex4_list", ncm_webcomindex4_list, 0); // 预园广告第三版-0617
		pasSetTcpFunName("ncm_webcomindex4_dr_images", ncm_webcomindex4_dr_images, 0);
		
		pasSetTcpFunName("ncm_webcommark_list", ncm_webcommark_list, 0); // 地图标签管理-0618
		
		pasSetTcpFunName("ncm_webcomindex5_list", ncm_webcomindex5_list, 0); // 太保广告管理-0620
		pasSetTcpFunName("ncm_webcomindex5_dr_images", ncm_webcomindex5_dr_images, 0);
		
		pasSetTcpFunName("ncm_webshopgtype_list", ncm_webshopgtype_list, 0); // 类别，商超管理表-0623
		
		pasSetTcpFunName("ncm_weboditydr_next", ncm_weboditydr_next, 0); // 商品导入-0808
