...
function HiddenDiv()
{
  ...
  for (i in goSelectTag)
  	//修改
  	//goSelectTag[i].style.visibility = "visible";
  	goSelectTag[i].style.visibility = "hidden";
  goSelectTag.length = 0;
...
function fToggleTags(){
  ...
  with (document.getElementsByTagName("SELECT")){
	//with (getElementsByTagName("SELECT")){
 	for (i=0; i<length; i++)
...
with (document) {
write("<Div id='VicPopCal' style='OVERFLOW:hidden;POSITION:absolute;VISIBILITY:hidden;border:0px ridge;z-index:100;width=100'>"); //border:2px ==>border:0px
...
write("<B style='cursor:hand' onclick='fSetDate(0,0,0)' onMouseOver='this.style.color=gcToggle' onMouseOut='this.style.color=gc_out'>清空</B>");
write("</td><td algin='center'>");
write("<B style='cursor:hand' onclick='fSetDate(giYear,giMonth,giDay)' onMouseOver='this.style.color=gcToggle' onMouseOut='this.style.color=gc_out'>今天: "+giYear+"-"+giMonth+"-"+giDay+"</B>");
write("</td></tr></table>");
write("</TD></TR>");
write("</TABLE></Div>");
}
