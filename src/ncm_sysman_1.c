...
int ncmSysUserlist_v4(utShmHead *psShmHead, int iFd,utMsgHead *psMsgHead)
{
    ...
    while(iReturn==0||iReturn==1405){
	  ...
	  iReturn = utMsgGetSomeNVar(psMsgHead,9,"groupid1",UT_TYPE_STRING,12,   caGroupid,
    ...
    for(i=0; i<lCount; i++){
	  	  unsigned long lAutodef = 0;
	  		...
	  		if(lAutodef > 0){
	  			  if(strlen(pOpt[i].opt)>0){ // 自定义二级以后
			  			  char tempcode[32] = "";
			  			  char caTempname[32] = "";
			  			  ncStrToStr(pOpt[i].opt, tempcode);
			  			  sprintf(caTemp, "select groupname from ncmautopri where groupcode = '%s'", tempcode);
			  		    pasDbOneRecord(caTemp,0,UT_TYPE_STRING,sizeof(caTempname)-1,caTempname);	
			  				utPltPutLoopVar(psDbHead,"opt",i+1,caTempname);
	  			  }else{  // 自定义一级
	  			  	  char caTempname[32] = "";
	  			  		sprintf(caTemp, "select groupname from ncmautopri where shopcode = '%s' and parentcode = '' ", pOpt[i].stype);
			  		    pasDbOneRecord(caTemp,0,UT_TYPE_STRING,sizeof(caTempname)-1,caTempname);	
			  				utPltPutLoopVar(psDbHead,"opt",i+1,caTempname);	
	  			  }
	  		}else{ // 不是自定义
...
int ncmDispRoleFunlist_v4(utShmHead *psShmHead, int iFd,utMsgHead *psMsgHead)
{
    ...
    unsigned long lIdd = 0;
	  dsCltGetSessionValue(1, "id", UT_TYPE_ULONG, sizeof(long), &lIdd);
	  dsCltGetSessionValue(1, "opt", UT_TYPE_STRING, sizeof(caOptd)-1, caOptd);
	  dsCltGetSessionValue(1, "stype", UT_TYPE_STRING, sizeof(caStyped)-1, caStyped);
	  printf("lIdd = %lu\n", lIdd);
	  printf("caOptd = %s\n", caOptd);
    ...
    if(strlen(caStyped)>0){
    	  sprintf(caSmtd,"select menuname from ncmdeptmenu where groupcode='%s'", caStyped);
    	  pasDbOneRecord(caSmtd,0,UT_TYPE_STRING,31,caName3);
    	  if(strcmp(caName3, "")!=0){
    	  	  if(strlen(caOptd)>0){
    	  	  		if(strlen(caOptd)>16){
	    	  	  			char optd[32] = "";
				    	  	  ...
				    	  	  if(strlen(optd)==2){
				    	  	  		sprintf(caSmt,"select menuname,submenu from ncmdeptmenu where groupcode='%s'", caStyped);		
				    	  	  }else{
				    	  	  		sprintf(caSmt, "select lright,submenu from dsroleattr where roleid in (select roleid from dsuserrole where id in (select id from dsuser where opt = '%s'))", caOptd);	
				    	  	  }
				    	  	  //printf("caSmt=%s  \n",caSmt);
...
int ncmRolelist_v4(utShmHead *psShmHead, int iFd,utMsgHead *psMsgHead)
{
    ...
    dsCltGetSessionValue(1,"dispname",UT_TYPE_STRING,30,caUsername2);
	  ...
	  iReturn = utMsgGetSomeNVar(psMsgHead,12, "limit",      UT_TYPE_STRING,10,  caLimit,
																             "page",       UT_TYPE_STRING,10,  caPage,  
																             "dir",        UT_TYPE_STRING,8,   caDir,
																             "keyword",    UT_TYPE_STRING,30,  caKeyword,
																             "selsid",     UT_TYPE_STRING,1000,caSelsid,			             
																             ...
																             "rightcheck", UT_TYPE_STRING,2048,caRightcheck,
    ...
    if(!utStrIsSpaces(caUserid)) { // 修改 
        ...
        if(iReturn == 0) {
            ...
            if(p) {
                ...
                utPltPutVar(psDbHead,"fac_desc",fac_desc);
                ...
                if(strlen(caOptd)>0){
		                if(strlen(caOpt)==2){
		                   ...
		                   pSelect=strdup(strUtlCvtInputSelect("rollevel","1",pBuf1,"changservice",70));
		                }
		                else if(strlen(caOpt)==4){
		                  strcpy(pBuf1,"地市级,2;区县级,3;"); 
		                  pSelect=strdup(strUtlCvtInputSelect("rollevel","2",pBuf1,"changservice",70));
		                }
		                else if(strlen(caOpt)==6){
		                  strcpy(pBuf1,"区县级,3;"); 
		                  pSelect=strdup(strUtlCvtInputSelect("rollevel","3",pBuf1,"changservice",70));
		                }
		                else{
		                  strcpy(pBuf1,"请选择, ;省级,1;地市级,2;区县级,3;"); 
		                  pSelect=strdup(strUtlCvtInputSelect("rollevel","",pBuf1,"changservice",70));
		                }
                }else{
                	  strcpy(pBuf1,"请选择, ;省级,1;地市级,2;区县级,3;"); 
                		if(strlen(caOpt)==2){
...
int ncmAdminUserSave(utShmHead *psShmHead, int iFd,utMsgHead *psMsgHead)
{
	        ...
	        if(lUserid > 0) {
	            ...
	            if(lCid==0 && strlen(caOptd)==0 && strlen(caStyped)==0){
	            		sprintf(caSql,"<DSSQL>insert into dsuser (id,name,dispname,groupid,groupname,addtime,\
	                           ...
	                           caKey,lTime,lUsrlevel,caEmail,caKey2,fac_desc,lcid,caStype);	
	            }else {  // 判断是否是二级分权
	            		sprintf(caSql,"<DSSQL>insert into dsuser (id,name,dispname,groupid,groupname,addtime,\
	                              moditime,lkey,lasttime,logcount,useflags,status,usrlevel,email,key2,company,cid,stype) values \
	                           (%lu,'%s','%s',%lu,'%s',%lu,%lu,'%s',%lu,0,0,0,%d,'%s','%s','%s',%d,'%s')</DSSQL>\n",
	                           lUserid,caUsername,caDispname,lGroupid,caGroupname,lTime,lTime,
	                           caKey,lTime,lUsrlevel,caEmail,caKey2,fac_desc,lCid,caStyped);	
	            }
	            printf("*******************开始2************************\n");
	            ...
	            //pasDbExecSqlF("update dsroleattr set pgroupcode='%s' where roleid=%d ",priv,atol(caChecked));
	            //pasDbExecSqlF("update dsrole set pgroupcode='%s' where id=%d ",priv,atol(caChecked));
	            ...
	            char *pUsername = NULL; //其它类似
				      ...
				      char caMaxmac[11] = "";
		      	  strcpy(caMaxmac,utComGetVar_sd(psShmHead,"NcmMaxUMac","0"));
	...
	//              ncWebAdminUserList(psShmHead, iFd,psMsgHead);
	        ...
	        if(lCid==0 && strlen(caOptd)==0 && strlen(caStyped)==0){
	        	  ...
	        	  utStrAddF(caSql,",moditime=%lu,lasttime=%lu,usrlevel=%lu,email='%s',company='%s',cid=%lu,stype='%s' where id=%lu </DSSQL>\n",
	                     lTime,lTime,lUsrlevel,caEmail,fac_desc,lCid,caStyped,lUserid);
	        }
	        ...
	        if(lUserid > 0) {
	            ...
	            if(lCid==0 && strlen(caOptd)==0 && strlen(caStyped)==0){
	            		sprintf(caSql,"<DSSQL>insert into dsuser (id,name,dispname,groupid,groupname,addtime,\
	                           ...
	                           lUserid,caUsername,caDispname,lGroupid,caGroupname,lTime,lTime,
	                           caKey,lTime,lUsrlevel,caEmail,fac_desc,lcid,caStype);	
	            }else{ // 判断是否是二级分权
	            		sprintf(caSql,"<DSSQL>insert into dsuser (id,name,dispname,groupid,groupname,addtime,\
	                              moditime,lkey,lasttime,logcount,useflags,status,usrlevel,email,company,cid,stype) values \
	                           (%lu,'%s','%s',%lu,'%s',%lu,%lu,'%s',%lu,0,0,0,%d,'%s','%s',%d,'%s')</DSSQL>\n",
	                           lUserid,caUsername,caDispname,lGroupid,caGroupname,lTime,lTime,
	                           caKey,lTime,lUsrlevel,caEmail,fac_desc,lCid,caStyped);
	            }
	            printf("*******************结束2************************\n");
	            utStrAddF(caSql,"<DSSQL>insert into dsuserrole(id,roleid,addtime,addby) values \
	                              (%lu,%lu,%lu,'%s')</DSSQL>\n",
	            ...
	            char *pUsername = NULL; //其它类似
		      	  ...
		      	  strcpy(caMaxmac,utComGetVar_sd(psShmHead,"NcmMaxUMac","0"));
				      ...
				      pDispname = strdup(caDispname);
				      pSsid = strdup(utComGetVar_sd(psShmHead,"NcmMySSID",  "ICPIC"));
				      ncSrvSynUserInfo(psShmHead,pGroupname,pSsid,pUsername,pPassword,NULL,pDispname,9,lMaxMac,0,&iStatus);
				      if(pSsid){
					         free(pSsid);
					         pSsid = NULL;
				      }
    ...
    if(!utStrIsSpaces(caCx)) {
        if(!utStrIsSpaces(caSear)) {
            ...
            if(IsInActionName(caSear)==0){
            	sprintf(caWhere,"and action='%s' ",ncsGetCodeByActionName(caSear));
            }
            else{
            	sprintf(caWhere,"and (sdate like %s or name like %s or ipaddr like %s or descr like %s)",
...
int ncmSysUserRoleConfig(utShmHead *psShmHead, int iFd,utMsgHead *psMsgHead)
{
    ...
    sprintf(caTemp,"select submenu from dsuser a,dsuserrole b,dsroleattr c where a.id=b.id and b.roleid=c.roleid and a.id=%d group by lright",lId);  
    ...
    //sprintf(caFun+strlen(caFun),",%s","单位管理");
...
int ncmWriteSysLog(utMsgHead *psMsgHead,char *username,char *action,char *result,char *msg)
{
  ...
  if(strlen(msg)!=0){
          ...
          iReturn = pasDbExecSqlF("insert into ncadminlog(sdate,name,ipaddr,descr,action,result) values ('%s','%s','%s','%s','%s','%s')",
          caDate,username,caIp,caMsg_t,action,result);
	    if(iReturn == 0) {
	        pasDbCommit(NULL);
	    }
  }
...
int ncmWebShowUserMan(utShmHead *psShmHead, int iFd,utMsgHead *psMsgHead)
{
    ...
    char users[100][20];
    long lLen,i;
    char caMac[20],caMac_all[256],mystype[32] = "";
       	    ...
       	    ncTimeToLong(expiretime, &l_expiretime);
		    	  ...
		    	  if(strlen(usermac) != 0){
					  		...
					  		while(q && *q){
					  				...
					  				sprintf(caTempd,"insert into ncsrvusermac (usermac, username, groupid, starttime, lasttime, moditime, flags) values ('%s', '%s', %lu, %lu, %lu, %lu, %lu)", caTemp2,username,atol(groupid),time(0),time(0),time(0),atol(flags));
						 				printf("caTempd = %s\n", caTempd);
...
//			 			pasDbExecSqlF(caTemp);
    ...
    pdate=(struct groupdate*)malloc(lTotRec*sizeof(struct groupdate)+1);
	  ...
	  if(strlen(caExport)>0){
	 		 strcpy(caFilename, "staffmanager.csv");
	 		 sprintf(caFile, "%s/%s", "/home/ncmysql/ncsrv/html/v8", caFilename);
	 		 printf("caFile = %s\n", caFile);
	 		 fp = fopen(caFile, "w");
	 		 if(fp == NULL){
	 		 		 pasDbCloseCursor(psCur);
    ...
    while(iReturn==0||iReturn==1405){
        ...
        if(strlen(caExport)>0){
				 		ncUtlPrintCsv(fp,8,UT_TYPE_STRING,caUsername,
		                            ...
		                            UT_TYPE_STRING,utTimFormat("%Y-%m-%d %H:%M:%S",lExpiretime),
		                            UT_TYPE_STRING,utTimFormat("%Y-%m-%d %H:%M:%S",lLasttime));
			  }
...
//#endif  
   ...
   printf("lCid = %d\n", lCid);
	 ...
	 if(strcmp(caKey, "update")==0&&strlen(caUserid)>0){
		    ...
		    utPltSetCvtHtml(1);
			  utPltPutVar(psDbHead,"userid",caUserid);
				...
				utPltSetCvtHtml(0);
				utPltPutVarF(psDbHead,"addtime","%lu",lAddtime);
				utPltPutVarF(psDbHead,"lasttime","%lu",lLasttime);
				...
				if(strlen(caGroupname)>16){
						...
						printf("groupcode = %s\n", groupcode);
						sprintf(caTemp, "select groupname from ncmautopri where groupcode = '%s'", groupcode);
						printf("caTemp = %s\n", caTemp);
    	  ...
    	  utPltPutVar(psDbHead,"usermac",caMac_all);
			  ...
			  if(strlen(caGroupcode)>0 || strlen(caGroupid)>0){ // 点击左边树
			  	  if(strlen(caGroupcode)>0){ // 是自定义
							  ...
							  sprintf(caTemp, "select groupname from ncmautopri where groupcode = '%s'", caGroupcode);
								printf("caTemp = %s\n", caTemp);
								pasDbOneRecord(caTemp,0,UT_TYPE_STRING,sizeof(groupcodename)-1, groupcodename);
								utPltPutVar(psDbHead,"groupcodename",groupcodename);
								printf("groupcodename = %s\n", groupcodename);
	 ...
	 utPltPutVar(psDbHead,"ssid",utComGetVar_sd(psShmHead,"NcmMySSID",  "ICPIC"));
  	...
  	if (strlen(groupcode) > 0)
    {
  	...
  	if(strlen(caStyped)>0){
	  		if(strlen(caOptd)>0){
	  				...
	  				sprintf(caTemp+strlen(caTemp)," and substring(groupname, 9, 2) = '%s' ", caStyped);
	  		}
  	}
  	printf("caTemp = %s\n", caTemp);
		pasDbOneRecord(caTemp,0,UT_TYPE_LONG,4,&lTotRec);
...
int ncm_webusermac_list(utShmHead *psShmHead, int iFd,utMsgHead *psMsgHead)
{
   ...
   if(strlen(usermac)>0){
			pasCvtGBK(2,usermac,caTemp,16);
			strcpy(usermac,caTemp);
   }
   if(strlen(username)>0){
			pasCvtGBK(2,username,caTemp,16);
   ...
   // 增加/修改
	 ...
	 // 查询
	 strcpy(sql, "select count(*) from ncsrvusermac u left join ncsrvgroup g on u.groupid=g.groupid  where 1=1");
	 if(!utStrIsSpaces(_keyword)) // 根据关键字查询
	 {
			snprintf(sql+strlen(sql), sizeof(sql)-strlen(sql), " and (u.username like '%%%s%%')", _keyword);
	 }
	 if (strlen(groupid) > 0)
   {	
   ...
   if (strlen(areaid) > 0)
   {
    	sprintf(sql+strlen(sql)," and left(g.groupname, 6) like '%lu%c' ", atol(areaid),'%');
   }
   printf("sql = %s\n", sql);
   ...
   if (strlen(areaid) > 0)
   {
	 ...
	 iret = pasDbFetchInto(psCur,UT_TYPE_STRING, sizeof(caUsermac)-1,   caUsermac,
	  ...
	  while(0 == iret || 1405 == iret){
			 ...
			 utPltPutLoopVar(psDbHead,"sdate",iNum,utTimFormat("%Y/%m/%d", lStarttime));
			 utPltPutLoopVar(psDbHead,"stime",iNum,utTimFormat("%H:%M:%S", lStarttime));
			 utPltPutLoopVar(psDbHead,"lasttime",iNum,s_lasttime);
			 utPltPutLoopVar(psDbHead,"edate",iNum,utTimFormat("%Y/%m/%d", lLasttime));
			 utPltPutLoopVar(psDbHead,"etime",iNum,utTimFormat("%H:%M:%S", lLasttime));
