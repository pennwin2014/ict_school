...
Ext.define('shops.advManager8',{
    ...
    initComponent : function(){
				...
				function gotoedit1(pos, view_long, view_width){
					...
					store_adsecord.load({params: {id:plateid}});
					  Ext.getCmp('advmanager5_detail').setVisible(true);
					  Ext.getCmp('advmanager5_intro').setVisible(false);
					  
					    Ext.getCmp('advmanager5_fp7').setVisible(false);
	    			  Ext.getCmp('advmanager5_sup').setVisible(true);
		 		 ...
		 		 var  store_adsecord = Ext.create('Ext.data.Store', {pageSize: 400,autoLoad:true,fields: ['plateid', 'level', 'imgpath'], autoLoad: false,remoteSort: true,
			      proxy: {type: 'ajax',url: '/pronline/Msg?FunName@ncmAdSecord',reader: {type:'json',root: 'eimdata',totalProperty: 'totalCount'},simpleSortMode: true
			      }});
			   
		 		 function gotoedit_s(plateid,level){
		 				
			       Ext.getCmp('advmanager5_fp7').setVisible(true);
	    			  Ext.getCmp('advmanager5_sup').setVisible(false);
	    			  
	    			  Ext.getCmp('plateid_s').setValue(plateid);
	    			  Ext.getCmp('level_s').setValue(level);
	    			  
	    			   Ext.getCmp('advmanager5_imgs').setSrc('');
	    			     Ext.getCmp('imgpath_s').setValue('');
	    			  
	    			  for(i=0;i<store_adsecord.getCount();i++){
	    			  	if(level==store_adsecord.getAt(i).get('level')){
	    			     Ext.getCmp('advmanager5_imgs').setSrc(store_adsecord.getAt(i).get('imgpath'));
	    			     Ext.getCmp('imgpath_s').setValue(store_adsecord.getAt(i).get('imgpath'));
	    			   }		  
              }
/*
					  for(i=1; i<store_shops_advmanager5.getCount()+1; i++){
						...
						items:[
						{
								...
								items:{
										...
										height:200,
							         				...
							         				el:{ 
																	click:function(){
															//			var plateid=Ext.getCmp('advmanager5_sid').value;
															       var level=Ext.getCmp('advmanager5_level').value;
					//										       alert(level);
					//													alert(plateid);
				                            	gotoedit_s(plateid, level);
					//													 
							         						}
							         				}
						         		}
		    ...
		    var fp2 = Ext.create('Ext.FormPanel', {
					...
					var fp7 = Ext.create('Ext.FormPanel', {
						id:'advmanager5_fp7',
    				hidden:true,
						frame:false,
						border:false,
						...
						items:[
					 {xtype: 'hidden',name: 'plateid', id:'plateid_s', value:''},
					 {xtype: 'hidden',name: 'level',id:'level_s',value:''},
					 {xtype: 'hidden',name: 'imgpath',id:'imgpath_s', value:''},

			    {
					  		...
					  		handler:function(){
					  			...
					  			var tlevel=Ext.getCmp('level_s').value;
					  			  ...
					  			  if(Ext.getCmp('plateid_s').value==''){
					  			  		alert("请先点击上面页面模板图片位置");
					  			  		return 0;	
					  			  }
					        ...
					        buttons: [{
					            ...
					            handler: function(thisbtn){
					                ...
					                //    store_adsecord.load({params: {id:tplateid,level:tlevel}});
					                   Ext.getCmp('advmanager5_imgs').setSrc(action.result.filepath);	
					                   Ext.getCmp('imgpath_s').setValue(action.result.filepath);	
        		...
        		windt.show();
		            ...
		            handler: function() {
		            	 if(Ext.getCmp('plateid_s').value==''){
					             ...
					             fp7.form.doAction('submit',{
							             url:'/pronline/Msg',
							             method:'post',
							             params:{FunName:'ncmAdSecord',update:'update'},
							             success:function(form,action){							           
							             	store_adsecord.load({params: {plateid:plateid}});
							             } 
					             })
			             }
		            }	
								...
								handler:function(){
									  if(confirm('您真的要执行删除操作吗？'))
									  {
									     ...
									     var imgpath=Ext.getCmp('imgpath_s').value;
									  	 store_adsecord.load({params: {update:'del',plateid:plateid,level:selsid,imgpath:imgpath},
									  	 	callback:function(records, options, success){
											  			 Ext.getCmp('advmanager5_imgs').setSrc('');	
											  		}});
									  	}
						...
						var fp4 = Ext.create('Ext.FormPanel', {
						id:'advmanager5_sup',
						hidden:true,
						frame:false,
						border:false,
						...
						items:[{
								...
								items:[{
										xtype:'label',
										html:'<div style="height:80px"><br>1.&nbsp;&nbsp;&nbsp;&nbsp;请先点击上面的图片，上传二级页面的图片<br><br>&nbsp;&nbsp;&nbsp;&nbsp;<br></div><br>'
								}]
						}]
				});
