...
//用户流量统计报表
var store_reports_trafficreport = Ext.create('Ext.data.Store', {pageSize: 25,id:'store_reports_trafficreport',fields: ['username', 'ssid', 'traffic','code'],remoteSort: true,autoLoad: false,
...
var store_charts_frequencyreport = Ext.create('Ext.data.Store', {pageSize: 10,id:'store_charts_frequencyreport',fields: [ {name:'chart_name',type:'string'}, 'inum', 'id', {name:'chart_number',type:'int'}, {name:'nets_subytes2',type:'string'},{name:'rate',type:'string'}],remoteSort: true,autoLoad: false,
	...
	store_charts_frequencyreport.on('beforeload', function (store, options) {
    Ext.apply(store.proxy.extraParams, store_charts_frequencyreport.new_params);
    });	
// 某一时间AP的MAC数量
...
//在线Mac   
var store_online_onlinemac = Ext.create('Ext.data.Store', {pageSize: 25,id:'store_online_onlinemac',fields: ['on_tMac', 'on_uMac', 'on_bSignal', 'on_sSignal','on_noise','on_channel','on_stime','on_etime','on_apAddr'],remoteSort: true,autoLoad: false,
...
//会员管理 
////商城用户管理
var store_shops_shopusermanager = Ext.create('Ext.data.Store', {pageSize: 25,id:'store_shops_shopusermanager',fields: ['uid', 'username', 'dispname', 'groupname', 'proname', 'usermac', 'password', 'nickname', 'membernum', 'integral', 'coupon', 'address', 'decript', 'lcount', 'imgpath', 'addtime', 'lasttime', 'flags','idno','sex','cname','email','mobile','stime'],idProperty: 'username',remoteSort: true,autoLoad: false,
			      proxy: {type: 'ajax',url: '/pronline/Msg?FunName@ncm_webcomshopuser_dns',reader: {type:'json',root: 'eimdata',totalProperty: 'totalCount'},simpleSortMode: true
...
//楼层管理
var store_shops_mallmanager = Ext.create('Ext.data.Store', {pageSize: 25,id:'store_shops_mallmanager',fields: ['fid', 'groupid', 'name', 'imgpath', 'detail', 'flags'],remoteSort: true,autoLoad: false,
			      proxy: {type: 'ajax',url: '/pronline/Msg?FunName@ncm_webreadfloor_list',reader: {type:'json',root: 'eimdata',totalProperty: 'totalCount'},simpleSortMode: true
			      },sorters: [{property: 'fid',direction: 'DESC'}]});
	store_shops_mallmanager.on('beforeload', function (store, options) {
    Ext.apply(store.proxy.extraParams, store_shops_mallmanager.new_params);
    });	
/*规则设置*/
//短信注册数量	
...
//参数设置	
var store_rules_srvpar = Ext.create('Ext.data.Store',{pageSize: 25,fields: ['pid','ptype','sessiontime','idletime','macbindtime','smslocalnum','smsinternum','smsusetime','limittime','limitflow','limitband','limitunit','passcodetime','servicetime','eng_smsmessage','zh_smsmessage','autoauth','macauth','weixinauth','maxmac','errornum','errorlock','addtime','moditime','paroption','flags','smsname'],autoLoad: false,id:'store_rules_srvpar',remoteSort: true,
...
//用户管理	
var store_users_staffmanager = Ext.create('Ext.data.Store',{pageSize: 25,fields: ['iNum', 'userid', 'username', 'dispname', 'groupname', 'groupid', 'ssid', 'proname', 'openid', 'usermac', 'macnum', 'password', 'mark', 'addtime', 'moditime', 'lasttime', 'expiretime', 'usertype', 'flags', 'maxbindmac','dev'],autoLoad: false,id:'store_users_staffmanager',remoteSort: true,
