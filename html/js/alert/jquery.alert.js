(function($){$.alerts={verticalOffset:0,horizontalOffset:0,repositionOnResize:true,overlayOpacity:.30,overlayColor:'#FFF',draggable:true,okButton:'确定',cancelButton:'取消',dialogClass:null,alert:function(message,title,callback){if(title==null)title='警告';
		...
		if(callback)callback(result);
			...
			$.alerts._show(title,message,null,null,null,'confirm',function(result){
				...
				$.alerts._show(title,message,value,null,null,'prompt',function(result){
					if(callback)callback(result);
					...
					},iframe:function(url,title,width,height){if(title==null)title='提示';
						...
						$.alerts._show(title,message,null,width,height,'choose',function(result){
							if(callback)callback(result);
								...
								$("BODY").append('<div id="popup_container" class="gSys_msg_box">'+'<div id="popup_title" class="ptitle"><span class="fn-bg rc-l"></span><h4></h4><span class="fn-bg rc-r"></span><a href="javascript:;" class="fn-bg aclose" title="close">关闭</a></div>'+'<div id="popup_message_box" class="pbox"><div class="pmsg"><b title="提示:" class="ico"></b><div class="ct" id="popup_message"></div></div></div>'+'<div id="popup_opt_box" class="popt"><div class="opt" id="opt"></div></div>'+'</div>');
								if($.alerts.dialogClass)$("#popup_container").addClass($.alerts.dialogClass);
								if(width==null){
									width=460;
								}
$("#popup_container").css({
	position:'absolute',zIndex:99999,padding:0,margin:0,width:width+'px'}
);
if(height){
	$("#popup_container").css("height",height+'px');
}
$("#popup_title h4").text(title);$("#popup_message_box b").addClass(type);$("#popup_message").text(msg);
...
switch(type){
	case'alert':$("#opt").html('<div id="popup_ok" class="pbtn" tabindex="0"><span>'+$.alerts.okButton+'</span></div>');
	...
	case'confirm':$("#opt").html('<div id="popup_cancel" class="pbtn" tabindex="0"><span>'+$.alerts.cancelButton+'</span></div><div id="popup_ok" class="pbtn" tabindex="0"><span>'+$.alerts.okButton+'</span></div>');
			$("#popup_ok").click(function(){
					$.alerts._hide();
	...
	case'iframe':$("#popup_message_box").remove();
					...
					iframe.src=msg;iframe.style.display="none";
	...
	case'prompt':$("#popup_message_box b").remove();
						$("#popup_message").attr("class","cp");
						$("#popup_message").append('<br /><input type="text" class="pinput" size="48" id="popup_prompt" />');$("#opt").html('<div id="popup_cancel" class="pbtn" tabindex="0"><span>'+$.alerts.cancelButton+'</span></div><div id="popup_ok" class="pbtn" tabindex="0"><span>'+$.alerts.okButton+'</span></div>');
						$("#popup_ok, #popup_cancel").mouseover(function(){
							$(this).attr("class","pbtn btn_hover");
						});
						$("#popup_ok, #popup_cancel").mouseout(function(){
...
//					var top=(($(window).height()/2)-($("#popup_container").outerHeight()/2))+$.alerts.verticalOffset;
					var top=0; //弹出popup顶端对齐
					var left=(($(window).width()/2)-($("#popup_container").outerWidth()/2))+$.alerts.horizontalOffset;
					if(top<0)top=0;if(left<0)left=0;top=top+$(window).scrollTop();
					...
					$("#popup_overlay").height($(document).height()
						...
						if($.alerts.repositionOnResize){
							switch(status){
								case true:$(window).bind('resize',$.alerts._reposition);break;case false:$(window).unbind('resize',$.alerts._reposition);
								break;
							}
						}
		}
