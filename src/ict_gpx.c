			...
			if(psCur)
			{
									  ...
									  UT_TYPE_STRING, sizeof(info) - 1, info)) || 1405 == iret)
				{
					...
					if(strlen(name) == 0 && strlen(info)==0 )
					{
						...
						snprintf(cainfo, sizeof(name)-1, "充值%.2f元", money/100.0);
					...
					else
					{
						...
						if(strcmp(info,"余额转出")==0)
						{
							utPltPutLoopVarF(psDbHead, "flag", iNum, "%d", 0);
							utPltPutLoopVar(psDbHead, "dtime", iNum, dtime);
							snprintf(cainfo, sizeof(cainfo)-1, "余额转出%.2f元", money/100.0);
							utPltPutLoopVar(psDbHead, "name", iNum, convert("GBK", "UTF-8", cainfo));
							utPltPutLoopVarF(psDbHead, "money", iNum, "%lu", money);
						}
						if(strcmp(info,"余额转入")==0)
						{
							utPltPutLoopVarF(psDbHead, "flag", iNum, "%d", 1);
							utPltPutLoopVar(psDbHead, "dtime", iNum, dtime);
							snprintf(cainfo, sizeof(cainfo)-1, "余额转入%.2f元", money/100.0);
							utPltPutLoopVar(psDbHead, "name", iNum, convert("GBK", "UTF-8", cainfo));
							utPltPutLoopVarF(psDbHead, "money", iNum, "%lu", money);
						}			
					}										
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
								...
								memset(sql, 0 , sizeof(sql));
								snprintf(caNamedes, sizeof(caNamedes)-1, "余额转出");
								...
								memset(sql, 0 , sizeof(sql));
								snprintf(caNamedes, sizeof(caNamedes)-1, "余额转入");
...
int ict_upgrade_package(utShmHead *psShmHead, int iFd, utMsgHead *psMsgHead)//升级套餐
{
    ...
    if(nowTime>(atoll(caTime) +65))//付款超时不能超过65秒
    {
        utPltPutVar(psDbHead, "tsid", caTsid);
        //utPltPutVarF(psDbHead, "money", "%lu", lmoney);
    ...
    else
    {
        ...
        pasDbOneRecord(sql, 0,
                       ...
                       UT_TYPE_LONG, 4, &Days,
                       UT_TYPE_LONG, 4, &Money);						   
        lmoney = (caDays * 24 * 3600 + startTime - atoll(caTime)) * caMoney / (caDays * 24 * 3600);//原套餐剩余钱
		nmoney = (caDays * 24 * 3600 + startTime - atoll(caTime)) * Money / (caDays * 24 * 3600);//升级套餐需要的钱
        money = nmoney - lmoney;//扣款
       ...
       //     pasDbExecSqlF(sql);
            ...
            snprintf(tableTime, sizeof(tableTime) - 1, "%s", utTimFormat("%Y%m", nowTime));
			char caNamedes[124]="";
			snprintf(caNamedes, sizeof(caNamedes)-1, "%s(升级)",namedes);
...
int ict_ncSrvLogOut(utShmHead *psShmHead, int iFd, utMsgHead *psMsgHead)
{
    ...
    else
    {
		...
		utPltPutVar(psDbHead, "mesg", convert("GBK", "UTF-8", "安全退出失败"));
		utPltPutVarF(psDbHead, "result", "%d", 3);
		utPltOutToHtml(iFd, psMsgHead, psDbHead, "school/safeExit/ict_safe_exit.htm");	
    }
...
int ict_getmobno_byusername(utShmHead *psShmHead, int iFd, utMsgHead *psMsgHead)//根据用户名查找手机号
{
	utMsgPrintMsg(psMsgHead);
    long wpDebug = utComGetVar_ld(psShmHead, "wpDebug", 0);
    char caVname[32] = "";
    ...
    if(strlen(caVname) > 0)
    {
        ...
        if(lCount > 0)
        {
            ...
            snprintf(caMsg, sizeof(caMsg) - 1, "找到用户名对应手机号");
            utPltPutVar(psDbHead, "mesg", convert("GBK", "UTF-8", caMsg));
			utPltPutVarF(psDbHead, "mobno", mobno);//找到用户名
            utPltPutVarF(psDbHead, "result", "%d", 0);//找到用户名
            utPltOutToHtml(iFd, psMsgHead, psDbHead, "school/findPassword/ict_find_mobno.htm");
        }
        else
        {
            snprintf(caMsg, sizeof(caMsg) - 1, "用户名不存在");
            utPltPutVar(psDbHead, "mesg", convert("GBK", "UTF-8", caMsg));
            utPltPutVarF(psDbHead, "result", "%d", 3);//用户名不存在
            utPltOutToHtml(iFd, psMsgHead, psDbHead, "school/findPassword/ict_find_mobno.htm");
        }
    }
    else
...
int ict_find_password(utShmHead *psShmHead, int iFd, utMsgHead *psMsgHead)
{
 ...
 //   char vName[32] = "";//用户名,对应ncsrvuser表中的username
	...
	{
		snprintf(caMsg, sizeof(caMsg) - 1, "账户名与验证码不匹配");
		utPltPutVar(psDbHead, "mesg", convert("GBK", "UTF-8", caMsg));
        utPltPutVarF(psDbHead, "result", "%d", 3);//账户名与验证码不匹配
   ...
   /* memset(sqlbuf, 0, sizeof(sqlbuf));
    ...
    if(iReturn != 0)
    {
		...
		utPltPutVar(psDbHead, "mesg", convert("GBK", "UTF-8", caMsg));
        utPltPutVarF(psDbHead, "result", "%d", 3);//密码修改失败,sql语句执行不成功
        utPltOutToHtml(iFd, psMsgHead, psDbHead, "school/findPassword/ict_change_password.htm");
    }
    else
    {
    	snprintf(caMsg, sizeof(caMsg) - 1, "密码找回成功");
		utPltPutVar(psDbHead, "mesg", convert("GBK", "UTF-8", caMsg));
