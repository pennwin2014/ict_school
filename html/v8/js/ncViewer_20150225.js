	      	...
	      	for(i=0;i<tab_len;i++){     		
				...
				if(tab_id=='ssjk'){
				    ...
				    if(right_sfun.indexOf("在线Mac")>0)tabs_center.add({autoScroll:true,id:tab_id+'_onlinemac',iconCls: 'onlineinfo',border:false,frame:false,title: "在线Mac"});
						...
						if(admincheck == "admin" || isexp == 'false')tabs_center.add({autoScroll:true,id:tab_id+'_staffmanager',iconCls: 'user',border:false,frame:false,title: "临时用户"});
				    ...
				    if(right_sfun.indexOf("上网人数流量报表")>0)tabs_center.add({autoScroll:true,id:tab_id+'_webreport',iconCls: 'report',border:false,frame:false,title: "上网人数流量报表"});
						...
						tabs_center.add({autoScroll:true,id:tab_id+'_frequencyreport',iconCls: 'report',border:false,frame:false,title: "到店频次统计",html: '<iframe  id="charts_smsrecount1" name="charts_smsrecount" src="/ncm_chart/frequencyreport.html" scrolling="no" width=100% height=950  FrameBorder=0 ></iframe>' });
