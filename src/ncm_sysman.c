...
int ncmSysUserlist_v4(utShmHead *psShmHead, int iFd, utMsgHead *psMsgHead)
{
    ...
    while((iReturn == 0 || iReturn == 1405) && iNum < lRowNum)
    {
        ...
        {
            case 1:
                utPltPutLoopVar(psDbHead, "level", iNum, "系统管理员");
                break;
            case 2:
                utPltPutLoopVar(psDbHead, "level", iNum, "普通管理员");
                break;
            case 3:
                utPltPutLoopVar(psDbHead, "level", iNum, "一般操作员");
                break;
            default:
                break;
        }
...
int ncmFirstMenuConfig(utShmHead *psShmHead, int iFd, utMsgHead *psMsgHead)
{
    ...
    for(i = 0; i < iNum; i++)
    {
        ...
        for(j = 0; j < oNum; j++)
        {
            ...
            {
                bflag = menu_ops[j].mflag;
                printf("mname=%s  opmname=%s\n", menu_struc[i].mname, menu_ops[j].mname);
                findflg = 1;
            }
        }
        if(findflg > 0)
...
int ncmWebShowUserMan(utShmHead *psShmHead, int iFd, utMsgHead *psMsgHead)
{
    ...
    while(iReturn == 0 || iReturn == 1405)
    {
        ...
        {
            case 0:
                strcpy(s_usertype, "固定用户");
                break;
            case 1:
                strcpy(s_usertype, "短消息");
                break;
            case 2:
                strcpy(s_usertype, "QQ");
                break;
            case 3:
                strcpy(s_usertype, "微信");
                break;
            default:
                break;
        }
